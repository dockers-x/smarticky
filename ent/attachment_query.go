...
func (_q *AttachmentQuery) QueryNote() *NoteQuery {
...
func (_q *AttachmentQuery) WithNote(opts ...func(*NoteQuery)) *AttachmentQuery {
	query := (&NoteClient{config: _q.config}).Query()
...
func (_q *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := _q.withNote; query != nil {
		if err := _q.loadNote(ctx, query, nodes, nil,
			func(n *Attachment, e *Note) { n.Edges.Note = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AttachmentQuery) loadNote(ctx context.Context, query *NoteQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Note)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attachment)
	for i := range nodes {
		if nodes[i].note_attachments == nil {
			continue
		}
		fk := *nodes[i].note_attachments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "note_attachments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
