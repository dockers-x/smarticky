...
func (e AttachmentEdges) NoteOrErr() (*Note, error) {
	if e.Note != nil {
		return e.Note, nil
...
func (*Attachment) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case attachment.ForeignKeys[0]: // note_attachments
	...
	for i := range columns {
		...
		case attachment.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				_m.note_attachments = new(uuid.UUID)
				*_m.note_attachments = *value.S.(*uuid.UUID)
			}
		case attachment.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_attachments", value)
			} else if value.Valid {
				_m.user_attachments = new(int)
				*_m.user_attachments = int(value.Int64)
			}
		default:
...
// QueryNote queries the "note" edge of the Attachment entity.
func (_m *Attachment) QueryNote() *NoteQuery {
	return NewAttachmentClient(_m.config).QueryNote(_m)
}
