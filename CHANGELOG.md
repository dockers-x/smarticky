...
- 将笔记本树直接整合到桌面侧栏，普通切换不再打开大型浮层。
- 新建笔记会立即使用当前笔记本；非笔记本视图默认创建到未归档。
- 将笔记卡片收敛为紧凑列表行，提高单屏可见笔记数量。
- 精简搜索、日历、过滤器、面包屑和批量操作的常驻占用。
- 保留可搜索的笔记本和标签选择器，用于移动、搜索及移动端导航。
- 增加快速按压反馈、短促弹层动效及减少动态效果适配。

### 验证 / Verification

- 新增笔记归属规则测试。
- 验证桌面、紧凑侧栏、移动端、亮暗主题和中英文布局。
